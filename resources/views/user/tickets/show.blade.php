<x-layouts.user :title="__('cinema.tickets.title')">
    <div class="mx-auto max-w-lg space-y-6">
        ...
        <section class="rounded-2xl border border-border bg-card p-6 text-center shadow-sm sm:p-8" data-ticket-card data-share-title="{{ $ticket->booking->screening?->movie?->title }}" data-calendar-title="{{ $ticket->booking->screening?->movie?->title }}" data-calendar-start="{{ $ticket->booking->screening?->starts_at?->utc()->toIso8601String() }}" data-calendar-end="{{ $ticket->booking->screening?->ends_at?->utc()->toIso8601String() }}" data-calendar-location="{{ $ticket->booking->screening?->room?->name }}">
            ...
            <div class="mt-6 flex flex-wrap justify-center gap-2">
                <button type="button" data-ticket-download class="rounded-xl border border-border px-3 py-2 text-sm font-semibold transition hover:border-primary">{{ __('cinema.tickets.download') }}</button>
                <button type="button" data-ticket-share class="rounded-xl border border-border px-3 py-2 text-sm font-semibold transition hover:border-primary">{{ __('cinema.tickets.share') }}</button>
                <button type="button" data-ticket-calendar class="rounded-xl border border-border px-3 py-2 text-sm font-semibold transition hover:border-primary">{{ __('cinema.tickets.calendar') }}</button>
            </div>
        </section>
    </div>
</x-layouts.user>
