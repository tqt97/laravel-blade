<x-layouts.user :title="__('cinema.screenings.title')">
    <div class="mx-auto max-w-6xl space-y-8">
        ...
        @if ($screenings->isEmpty())
            <div class="rounded-2xl border border-border bg-card p-10 text-center text-sm text-muted-foreground">{{ __('cinema.screenings.empty') }}</div>
        @else
                ...
                @foreach ($screenings as $screening)
                    <article class="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                        ...
                        <div class="space-y-3 p-5">
                            ...
                            <p class="text-sm text-muted-foreground">{{ $screening->room?->name }} · {{ $screening->starts_at->timezone($screening->room?->timezone ?? config('app.timezone'))->format('d/m/Y H:i') }}</p>
                            <x-admin.button :href="route('user.screenings.show', $screening)" icon="eye">{{ __('cinema.screenings.choose_seats') }}</x-admin.button>
                        </div>
                    </article>
                @endforeach
            </div>
