<x-layouts.movie :title="__('booking.combos.title')">
    <div class="mx-auto max-w-3xl space-y-8 px-5 py-12 sm:px-8">
        <div>
            ...
            <p class="mt-2 text-sm text-muted-foreground">{{ __('booking.combos.description') }}
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('user.bookings.combos.store', $booking) }}"
            ...
            @forelse ($concessions as $concession)
                ...
                <label
                    class="flex items-center gap-4 rounded-xl border border-border bg-background p-4 transition has-[:focus-visible]:border-primary has-[:focus-visible]:ring-4 has-[:focus-visible]:ring-primary/10">
                    @if ($concession->image_url)
                        <img src="{{ $concession->image_url }}" alt="{{ $concession->name }}"
                            class="size-20 shrink-0 rounded-xl object-cover" loading="lazy">
                    @else
                        <div class="grid size-20 shrink-0 place-items-center rounded-xl bg-primary-soft text-3xl"
