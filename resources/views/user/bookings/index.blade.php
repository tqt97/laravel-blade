<x-layouts.user :title="__('booking.bookings.title')">
    <div class="mx-auto max-w-6xl space-y-8">
        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <x-admin.button :href="route('cinema.movies.index')" icon="arrow-right">
                {{ __('cinema.public.movies') }}
            </x-admin.button>
        </div>
        <form method="GET" class="flex flex-wrap items-center gap-3" aria-label="{{ __('booking.bookings.status') }}">
            <label for="booking-status" class="text-sm font-semibold">
                {{ __('booking.bookings.status') }}
            </label>
            ...
            <div class="grid gap-3 sm:hidden">
                @foreach ($bookings as $booking)
                    <a href="{{ route('user.bookings.show', $booking) }}"
                        class="rounded-2xl border border-border bg-card p-4 shadow-sm transition hover:border-primary">
                        <div class="flex items-start justify-between gap-3">
                            <span class="font-semibold">
                                {{ $booking->screening?->movie?->title ?? '—' }}</span>
                        ...
                        <p class="mt-2 text-xs text-muted-foreground">{{ $booking->items_count }}
                    ...
                    <table class="min-w-full divide-y divide-border text-left text-sm">
                        <thead class="bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground">
                            <tr>
                                ...
                                <th class="whitespace-nowrap px-5 py-4 text-right">{{ __('booking.bookings.total') }}
                                </th>
                                <th class="whitespace-nowrap px-5 py-4 text-right">{{ __('booking.bookings.actions') }}
                                </th>
                            </tr>
                        </thead>
                            ...
                            @foreach ($bookings as $booking)
                                <tr class="align-top">
                                    <td class="px-5 py-4 font-semibold">
                                        {{ $booking->screening?->movie?->title ?? '—' }}</td>
                                    <td class="whitespace-nowrap px-5 py-4 text-muted-foreground">
                                        {{ $booking->screening?->starts_at?->timezone(config('app.timezone'))->format('d/m/Y H:i') ?? '—' }}<br>{{ $booking->screening?->ends_at?->timezone(config('app.timezone'))->format('H:i') ?? '—' }}
                                    </td>
                                    <td class="whitespace-nowrap px-5 py-4 text-muted-foreground">
                                    ...
                                    <td class="whitespace-nowrap px-5 py-4 text-right">
                                        <x-admin.button :href="route('user.bookings.show', $booking)" variant="ghost" icon="eye" icon-only
                                            :title="__('booking.bookings.details')"
