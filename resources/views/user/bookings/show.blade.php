<x-layouts.user :title="__('booking.bookings.details')">
    <div class="mx-auto max-w-3xl space-y-8">
        ...
        @php
            ...
            $timezone = $booking->screening?->room?->timezone ?? config('app.timezone');
        @endphp
        <section class="space-y-6 rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
            ...
            <div class="border-t border-border pt-6">
                ...
                <div class="mt-3 grid gap-3 rounded-xl bg-muted p-4 text-sm sm:grid-cols-3">
                    <div><p class="text-xs text-muted-foreground">{{ __('booking.bookings.date') }}</p><p class="mt-1 font-semibold">{{ $booking->screening?->starts_at?->timezone($timezone)->format('d/m/Y') ?? '—' }}</p></div>
                    <div><p class="text-xs text-muted-foreground">{{ __('booking.bookings.room') }}</p><p class="mt-1 font-semibold">{{ $booking->screening?->room?->name ?? '—' }}</p></div>
                    <div><p class="text-xs text-muted-foreground">{{ __('booking.bookings.booking_id') }}</p><p class="mt-1 font-semibold">#{{ $booking->id }}</p></div>
                </div>
            </div>
            <div class="border-t border-border pt-6">
                <h3 class="text-sm font-semibold">{{ __('booking.success.summary') }}</h3>
                <dl class="mt-3 space-y-3 text-sm">
                    <div class="flex justify-between gap-4"><dt class="text-muted-foreground">{{ __('booking.bookings.seats') }}</dt><dd class="font-semibold">{{ \App\Support\Money\Money::fromMinorUnits($seatTotal, $currency)->format() }}</dd></div>
                    <div class="flex justify-between gap-4"><dt class="text-muted-foreground">{{ __('booking.bookings.combos') }}</dt><dd class="font-semibold">{{ \App\Support\Money\Money::fromMinorUnits($comboTotal, $currency)->format() }}</dd></div>
                    @if ((int) $booking->discount_minor_units > 0)<div class="flex justify-between gap-4 text-success"><dt>{{ __('booking.checkout.discount') }}</dt><dd>-{{ \App\Support\Money\Money::fromMinorUnits((int) $booking->discount_minor_units, $currency)->format() }}</dd></div>@endif
                    <div class="flex justify-between gap-4 border-t border-border pt-3 text-base"><dt class="font-semibold">{{ __('booking.bookings.total') }}</dt><dd class="font-bold text-primary">{{ \App\Support\Money\Money::fromMinorUnits((int) $booking->total_minor_units, $currency)->format() }}</dd></div>
                </dl>
            </div>
        ...
        <div id="cancel-booking-modal" data-modal hidden class="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4 backdrop-blur-sm">
            <div class="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl" role="dialog" aria-modal="true" aria-labelledby="cancel-booking-title">
                <h2 id="cancel-booking-title" data-modal-title class="text-lg font-semibold">{{ __('booking.bookings.cancel') }}</h2>
                <p data-modal-description class="mt-2 text-sm leading-6 text-muted-foreground">{{ __('booking.admin.cancel_confirm') }}</p>
                <label class="mt-5 block text-sm font-semibold" for="cancel-reason">{{ __('booking.bookings.reason') }}</label>
                <textarea id="cancel-reason" data-modal-input name="reason" rows="3" maxlength="500" class="mt-2 w-full rounded-xl border border-border bg-background p-3 text-sm" placeholder="{{ __('booking.bookings.reason') }}"></textarea>
                <div class="mt-6 flex justify-end gap-2"><button type="button" data-modal-close class="rounded-xl border border-border px-3.5 py-2 text-sm font-semibold">{{ __('ui.actions.cancel') }}</button><button type="button" data-modal-confirm class="rounded-xl bg-destructive px-3.5 py-2 text-sm font-semibold text-destructive-foreground"><span data-modal-label>{{ __('ui.actions.confirm') }}</span></button></div>
            </div>
        </div>
    </div>
</x-layouts.user>
