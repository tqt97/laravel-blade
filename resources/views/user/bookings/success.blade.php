<x-layouts.storefront :title="__('booking.success.title')">
    <div class="mx-auto max-w-3xl space-y-8 px-5 py-12 sm:px-8">
        ...
        <section class="rounded-2xl border border-success/30 bg-success-soft p-6 text-center sm:p-10">
            ...
            <p class="mt-4 text-sm font-semibold text-success-foreground">{{ __('booking.bookings.booking_id', ['id' => $booking->id]) }}</p>
        </section>
        ...
        <section class="rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
            ...
            <div class="mt-6 grid gap-4 rounded-xl bg-muted p-4 text-left sm:grid-cols-3">
                <div><p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{{ __('booking.bookings.date') }}</p><p class="mt-1 text-sm font-semibold">{{ $booking->screening?->starts_at?->timezone($timezone)->format('d/m/Y') ?? '—' }}</p></div>
                <div><p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{{ __('booking.bookings.schedule') }}</p><p class="mt-1 text-sm font-semibold">{{ $booking->screening?->starts_at?->timezone($timezone)->format('H:i') ?? '—' }} – {{ $booking->screening?->ends_at?->timezone($timezone)->format('H:i') ?? '—' }}</p></div>
                <div><p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{{ __('booking.bookings.room') }}</p><p class="mt-1 text-sm font-semibold">{{ $booking->screening?->room?->name ?? '—' }}</p></div>
            </div>
