    ...
    <div class="mx-auto max-w-6xl space-y-8 px-5 py-12 sm:px-8" data-booking-checkout
        ...
        <div>
            @if ($booking->status === \App\Enums\Movie\Booking\BookingStatus::Held)
                <a href="{{ route('cinema.screenings.show', [$booking->screening->movie, $booking->screening]) }}" class="text-sm font-semibold text-primary hover:underline">← {{ __('booking.checkout.edit_selection') }}</a>
            @else
        ...
        <form id="booking-payment-form" method="POST" action="{{ route('user.bookings.pay', $booking) }}" data-payment-form data-processing-label="{{ __('booking.checkout.processing') }}">
            @csrf
        <div class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <main class="space-y-6">
                ...
                <section class="space-y-4 rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
                    <div><h2 class="text-lg font-semibold">{{ __('booking.checkout.combos') }}</h2><p class="mt-1 text-sm text-muted-foreground">{{ __('booking.checkout.combos_locked') }}</p></div>
                    @if ($booking->concessions->isNotEmpty())
                        <div class="divide-y divide-border rounded-xl border border-border bg-background">
                            @foreach ($booking->concessions as $line)
                                <div class="flex items-center justify-between gap-4 p-4 text-sm"><div class="min-w-0"><p class="font-semibold">{{ $line->concession?->name ?? '—' }}</p><p class="mt-1 text-xs text-muted-foreground">{{ $line->quantity }} × {{ \App\Support\Money\Money::fromMinorUnits((int) $line->unit_price_minor_units, $currency)->format() }}</p></div><span class="shrink-0 font-semibold">{{ \App\Support\Money\Money::fromMinorUnits((int) $line->total_minor_units, $currency)->format() }}</span></div>
                            @endforeach
                        </div>
                    @else
                        <p class="rounded-xl border border-dashed border-border p-4 text-sm text-muted-foreground">{{ __('booking.success.no_combos') }}</p>
                    @endif
                </section>
            </main>
            ...
            <aside class="sticky bottom-4 z-20 h-fit space-y-4 lg:top-24 lg:bottom-auto">
                ...
                <section class="rounded-2xl border border-border bg-card p-5 shadow-sm"><label for="promo-code" class="text-sm font-semibold">{{ __('booking.checkout.promo_title') }}</label><p class="mt-1 text-xs leading-5 text-muted-foreground">{{ __('booking.checkout.promo_description') }}</p><div class="mt-3 flex gap-2"><input id="promo-code" name="code" type="text" maxlength="32" value="{{ old('code', $booking->coupon_code) }}" class="min-w-0 flex-1 rounded-xl border border-border bg-background px-3 py-2 text-sm" placeholder="{{ __('booking.checkout.promo_placeholder') }}" aria-describedby="promo-hint"><button type="submit" formmethod="POST" formaction="{{ route('user.bookings.coupon.apply', $booking) }}" formnovalidate class="rounded-xl bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground">{{ __('booking.checkout.promo_apply') }}</button></div><p id="promo-hint" class="mt-2 text-xs text-muted-foreground">{{ $booking->coupon_code ? __('booking.checkout.promo_applied', ['code' => $booking->coupon_code]) : __('booking.checkout.promo_hint') }}</p>@error('code')<p class="mt-2 text-xs text-danger" role="alert">{{ $message }}</p>@enderror</section>
