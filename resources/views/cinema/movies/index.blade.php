<x-layouts.movie :title="__('cinema.public.movies')" :description="__('cinema.public.hero_description')">
    <section class="relative isolate overflow-hidden bg-slate-950 text-white">
        <div class="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_80%_20%,rgb(37_99_235_/_0.28),transparent_32%),radial-gradient(circle_at_10%_100%,rgb(124_58_237_/_0.25),transparent_35%)]"></div>
    ...
    <section id="showtimes" class="mx-auto max-w-7xl space-y-8 px-5 py-12 sm:px-8">
        ...
        <form method="GET" action="{{ route('cinema.movies.index') }}" class="flex flex-col gap-2 sm:flex-row sm:items-end"
            role="search">
            <div class="min-w-0 flex-1">
                <label for="movie-search" class="text-sm font-semibold">{{ __('cinema.public.search_label') }}</label>
            ...
            <x-admin.button type="submit" icon="search" class="sm:shrink-0">{{ __('cinema.public.search_submit') }}</x-admin.button>
        ...
        </div>@else<div class="grid gap-x-5 gap-y-8 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($movies as $movie)
                    <article
                        class="group overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
                        <a href="{{ route('cinema.movies.show', $movie) }}" class="relative block overflow-hidden bg-muted">
                            @if ($movie->poster_url)
                                <img src="{{ $movie->poster_url }}" alt="{{ $movie->title }}"
                            width="440" height="660" loading="lazy" decoding="async" class="aspect-[2/3] w-full object-cover transition duration-500 group-hover:scale-105">@else<div
                                    class="flex aspect-[2/3] items-center justify-center bg-muted text-5xl">🎬</div>
                            @endif
                            <span class="absolute left-3 top-3 rounded-md bg-slate-950/80 px-2 py-1 text-xs font-bold text-white backdrop-blur">{{ $movie->rating ?: 'PG' }}</span>
                        </a>
