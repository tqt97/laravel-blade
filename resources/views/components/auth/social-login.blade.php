...
@else
    <button type="button" disabled aria-disabled="true" title="Cấu hình route {{ $routeName }} để bật đăng nhập {{ $label }}" class="flex cursor-not-allowed items-center justify-center gap-3 rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm font-semibold text-neutral-400 opacity-75 dark:border-white/10 dark:bg-white/[0.06] dark:text-neutral-500">
@endif
        @if ($provider === 'google')
        ...
        @endif
        <span>{{ $label }}</span>
        @unless ($providerRouteAvailable)<span class="ml-auto text-[10px] font-medium uppercase tracking-wide text-neutral-400">Sắp có</span>@endunless
